...
class AiMeterTui(App[None]):
    ...
    def _render_snapshot(self, force: bool = False) -> None:
        ...
        if render_ai and self._show_claude:
            ...
            if api is not None:
                five_h_line = self._render_api_limit(
                    api.five_hour_pct, api.five_hour_reset_secs, limit_w
                )
                seven_d_line = self._render_api_limit(
                    api.seven_day_pct, api.seven_day_reset_secs, limit_w
                )
                ...
                if self.engine.claude_api is None:
                    five_h_line = "[yellow]unknown[/] [grey40](usage-api off)[/]"
                    seven_d_line = "[yellow]unknown[/] [grey40](usage-api off)[/]"
