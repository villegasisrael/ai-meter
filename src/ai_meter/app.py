...
from ai_meter.hardware.models import HardwareMetric
...
class DashboardSnapshot:
    ...
    hardware_metrics: list[dict[str, Any]] = field(default_factory=list)
    hardware_provider_status: dict[str, dict[str, Any]] = field(default_factory=dict)
...
class MonitorEngine:
    def __init__(
        ...
        hardware_config = self.config.providers.hardware
        amd_probe_path = hardware_config.amd_probe_path.strip() or None
        self.cpu_identity = detect_cpu_identity()
    ...
    def run_heavy_collection(self) -> None:
        try:
            system_batch = self.system_collector.collect(include_temp=False, include_top=True)
    ...
    def run_hardware_collection(self) -> None:
        if not self.config.providers.hardware.enabled:
            return
        try:
            self.hardware_registry.collect()
        except Exception as exc:
            self._persist_error("hardware", str(exc))

    def run_full_collection(self) -> None:
        self.run_light_collection()
        self.run_hardware_collection()
    ...
    def snapshot(self) -> DashboardSnapshot:
        hardware_metrics = self.hardware_registry.snapshot()
        hardware_status = self.hardware_registry.status_snapshot()
        with self._lock:
            system_meta = self._merge_hardware_meta(
                self._provider_meta.get("system", {}),
                hardware_metrics,
                hardware_status,
            )
            return DashboardSnapshot(
                ...
                hardware_metrics=[
                    metric.model_dump(mode="json") for metric in hardware_metrics
                ],
                hardware_provider_status=hardware_status,
    ...
    @staticmethod
    def _merge_hardware_meta(
        base: dict[str, Any],
        metrics: list[HardwareMetric],
        statuses: dict[str, dict[str, Any]],
    ) -> dict[str, Any]:
        meta = dict(base)
        meta["hardware_metrics"] = [
            metric.model_dump(mode="json") for metric in metrics
        ]
        meta["hardware_provider_status"] = statuses
        ...
        if cpu_metrics:
            cpu = cpu_metrics[0]
            meta["cpu_temp_c"] = cpu.value
            meta["cpu_temp_source"] = cpu.source
            meta["needs_admin"] = False
        else:
            source = _missing_cpu_source(statuses)
            meta["cpu_temp_c"] = None
            meta["cpu_temp_source"] = source
            meta["needs_admin"] = source == "wmi:admin_required"

        if gpu_metrics:
            gpu = gpu_metrics[0]
            meta["gpu_temp_c"] = gpu.value
            meta["gpu_name"] = gpu.label
            meta["gpu_temp_source"] = gpu.source
            meta["gpu_temps"] = [
                {
                    "name": metric.label,
                    "temp_c": metric.value,
            ...
            meta["gpu_name"] = None
            meta["gpu_temp_source"] = "unknown"
            meta["gpu_temps"] = []
        return meta


def _missing_cpu_source(statuses: dict[str, dict[str, Any]]) -> str:
    platform = statuses.get("platform")
